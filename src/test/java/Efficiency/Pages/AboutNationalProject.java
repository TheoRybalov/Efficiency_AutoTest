...
public class AboutNationalProject extends CommonFunctions {
    ...
    public void PassportTargetedSupportLink_Redirect(String mode){
        if(!Objects.equals(mode, "headless")){
            ...
            super.CheckDownloadedByLinkFile(PassportTargetedSupportLink, "Паспорт-L2-ФП Адресная поддержка.pdf");
        }
    }

    @Step("Проверка редиректа по ссылке 'Паспорт федерального проекта «Системные меры по повышению производительности труда» ↗'")
    public void PassportSystemicMeasuresLink_Redirect(String mode){
        if(!Objects.equals(mode, "headless")){
            ...
            super.CheckDownloadedByLinkFile(PassportSystemicMeasuresLink, "Паспорт-L1-ФП Системные меры.pdf");
        }
    }

    @Step("Проверка редиректа по ссылке 'Паспорт Национального проекта «Производительность труда» ↗'")
    public void PassportLaborProductivityLink_Redirect(String mode){
        ...
        if(!Objects.equals(mode, "headless")){
            ...
            super.CheckDownloadedByLinkFile(PassportLaborProductivityLink, "Паспорт_L_НП_Производительность_труда.pdf");
        }
    }
